...
export default function BookingUser ({ userdata }) {
  const user = useSelector(state => OneUser(state, userdata))
  const auth = useSelector(state => OneAuth(state, userdata))
  const bookingcount = UserBookCount(userdata)
      ...
      <MDBCardBody className='p-4'>
        <div className='d-flex flex-column text-black align-items-center'>
          <div onClick={onClickHandle} className='position-relative'>
            ...
            {auth?.isOnline ? (
              <MDBBadge
                className='position-absolute top-100 start-100 translate-middle p-1 bg-success
...
border border-light rounded-circle'
          ...
          <div className='mt-2'>
            ...
            <MDBCardText
              className='text-lead text-center text-nowrap'
              href={`mailto:${user?.email ?? './'}`}
            >
              {user?.email ?? 'Suganthy@gmail.com'}
            </MDBCardText>
            <MDBCardText className='text-lead text-center'>
              {user?.mobile}
            </MDBCardText>
            ...
            >
              <div>
                ...
                <p className='mb-0 text-center'>{bookingcount ?? 1}</p>
              </div>
              <div className='px-3'>
                <p className='small text-muted mb-1'>Join With Us</p>
                <p className='mb-0 small text-wrap'>
                  {new Date(auth?.join).toDateString() ?? '11/10/2023'}
                </p>
              </div>
            </div>
            <div className='d-flex pt-1'>
              <MDBBtn outline className='me-1 flex-grow-1' onClick={OnClick}>
