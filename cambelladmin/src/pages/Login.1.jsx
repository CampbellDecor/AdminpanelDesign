...
export function Login() {
  const { cambell, setLogin, setCurrentUser, islogin } = useContext(
    CambellContext
  );
  ...
  const onSubmit = async e => {
    ...
    try {
      if (loginUser.email && loginUser.password) {
        const loguser = signInWithEmailAndPassword(
          auth,
          loginUser.email,
          loginUser.password
        );
        ...
        if ((await loguser).user.emailVerified === false) {
          ...
          const admincol = await doc(
            fstore,
            "admins",
            (await loguser).user.uid
          );
  ...
  return islogin
    ...
    : <MDBContainer fluid className="vh-100 login">
        ...
        <MDBCard
          className="mx-auto p-5 shadow-5 col-12 col-sm-10 col-md-8 col-lg-5"
          style={{
            marginTop: "-150px",
        ...
        >
          <MDBCardBody className="p-5 text-center">
            ...
            <MDBValidation>
              ...
              <MDBBtn className="w-50 mb-4" size="md" onClick={onSubmit}>
                Log In
              </MDBBtn>
            </MDBValidation>
            <div>
